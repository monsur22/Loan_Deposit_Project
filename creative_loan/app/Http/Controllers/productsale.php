...
class productsale extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.productsales.productsalesindex', ['customers' => $customers, 'products' => $carts->items, 'totalPrice' => $carts->totalPrice, 'totalCost' => $carts->totalCost]);
    }
    public function fetch(Request $request)
    {
        if ($request->get('query')) {
            $query = $request->get('query');
            $data = DB:: table('stock_reports')
...
{
    $id = $request->id;
    $productByDesc = DB::table('stock_reports')->where('report_stock_id', $id)->first();
...
public function getRemoveItem($storid)
{
    $oldCart = Session::has('carts') ? Session::get('carts') : null;
    $carts = new Carts($oldCart);
    $carts->removeItem($storid);
...
//            'saleInvoice_date' => $request->input('saleInvoice_date')
        ...
        DB::table('stock_reports')->where('report_stock_id',$product['item']->report_stock_id)
...
//--------------------For sell Invoice Print-----------------------------------------------------
        $commanyinfos = DB::table('companies')->get();
...
public function productSaleReturn(Request $request){
    ...
    $customers = DB::table('custormers')->get();
    view()->share('customers',$customers);
...
public function salesdateWiseReport(Request $request)
{
    ...
    return view('admin.productsales.productsale_report');
}

public  function salesinvoiceInfo($invoice){
    $companydetails=company::all();
    view()->share('companydetails', $companydetails);
   $invoiceInfos = DB::table('sale_invoices')
        ->where('saleInvoice_no',$invoice)
        ->get();
    $sales_report =  DB::table('sale_items')
        ->where('saleInvoice_no',$invoice)
        ->get();
    // $sales_report=SaleItem::find($invoice);
    // $customers = DB::table('customer')
    //     ->where('saleInvoice_no',$invoice)
    //     ->join('sale_invoices','customer.customer_name','=','sale_invoices.saleInvoice_customerName')
    //     ->select('customer.*')
    //     ->get();
    view()->share('sales_report',$sales_report);

    $subtotalSum = $sales_report->sum('saleItem_quantity');
    view()->share('subtotalSum', $subtotalSum);

    $totalSum = $sales_report->sum('saleItem_total');
    view()->share('totalSum', $totalSum);


    return view('admin.productsales.sale_report_by_invoice',[
...
public function sales_return_report(Request $request)
{
    ...
    return view('admin.productsales.return_sales_report');
}

public function sales_return_report_by_id($invoice)
{
    $companydetails=company::all();
    view()->share('companydetails', $companydetails);
    ...
    return view('admin.productsales.return_sale_report_by_id');
}
