...
<div class="container text-center">
    ...
    <h6>Phone:{{$item->company_mobile}} &nbsp Email:{{$item->company_email}}</h6>
        @endforeach
<br>
<h3> <u>Purches Return Report</u> </h3>
    ...
    <div class="ibox"style="margin-top: 5px;">
            ...
            <div class="ibox-head">
                    ...
                    <form action="{{url('/purches-return-reoprt')}}" method="get">
                                {{ csrf_field() }}
                                <div class="row">
            ...
            <div class="ibox-body">
                    <table class="table table-striped table-bordered table-hover" id="example-table" cellspacing="0" width="100%">
                        ...
                        <tbody>
                            ...
                            <tr>
                            <td>{{$i++}}</td>
                            <td>{{$item->PurchaseReturnDate}}</td>
                            <td>{{$item->PurchaseReturnNo}}</td>
                            
                            {{-- <td>{{$item->sid}}</td> --}}
                            <td>{{$item->StockId}}</td>
                            <td>{{$item->purchaseReturn_qty}}</td>

                            <td>{{$item->purchaseReturn_total}}</td>
                            
                            <td>
                                <a href="{{url('purches-return-reoprt-by-Id/'.$item->PurchaseReturnNo)}}" class="btn btn-info" title="Report">
